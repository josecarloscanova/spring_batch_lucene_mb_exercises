...
public class ArtistCreditNameBootLoader {
	...
	public static void main(String[] args)  {
		String [] baseContext = new String[] {
				...
				"spring-batch-file-jobs/database/artist_credit_name_database_job.xml"};
		...
		try { 
			...
			Set<Long> executions = jobOperator.getRunningExecutions("artistCreditNameDatabaseJob");
...
//				jobOperator.stop(it.next());  
			...
			jobOperator.start("artistCreditNameDatabaseJob", "");
//			SimpleJobLauncher jobLauncher = (SimpleJobLauncher) ctx.getBean("jobLauncher");
//			jobLauncher.run(artistAliasJob, new JobParameters());
		}catch (Exception ex) 
		{ 
			ex.printStackTrace();
		}
