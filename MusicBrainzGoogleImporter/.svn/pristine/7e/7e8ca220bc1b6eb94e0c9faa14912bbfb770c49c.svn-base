package org.nanotek.musicbrainz.job.lucene;
...
public class LabelNameBootLoader {
	...
	public static void main(String[] args)  {
		...
		"/spring-batch-file-jobs/label_name_index_writer_job.xml"};
		ctx = new ClassPathXmlApplicationContext(baseContext);
//		Job artis = (Job) ctx.getBean("artistNameIndexJob");
		...
		try { 
			...
			Set<Long> executions = jobOperator.getRunningExecutions("labelNameIndexJob");
...
//				jobOperator.stop(it.next());  
			...
			jobOperator.start("labelNameIndexJob", "");
