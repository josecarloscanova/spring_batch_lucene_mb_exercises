...
public class CsvToBean<M extends MappingStrategy<T> , T> {
    ...
    public List<T> parse(M mapper, Reader reader) {
        return parse(mapper, new CSVReader(reader));
    }

    public List<T> parse(M mapper, CSVReader csv) {
        try {
    ...
    public T processLine(M mapper, String[] line) 
