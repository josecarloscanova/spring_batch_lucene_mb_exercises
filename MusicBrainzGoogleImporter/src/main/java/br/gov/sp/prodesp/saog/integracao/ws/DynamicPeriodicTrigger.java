...
public class DynamicPeriodicTrigger implements Trigger {
	...
	public Date nextExecutionTime(TriggerContext triggerContext) {
		Long variableRate = Math.round(Math.random() * ((this.fixedRate) ? this.period : 10000)); 
		if (triggerContext.lastScheduledExecutionTime() == null) {
			return new Date(System.currentTimeMillis() + this.initialDelay + variableRate);
		}
		else if (this.fixedRate) {
			return new Date(triggerContext.lastScheduledExecutionTime().getTime() + this.period + variableRate);
		}
		return new Date(triggerContext.lastCompletionTime().getTime() + (this.period + variableRate));
	}
